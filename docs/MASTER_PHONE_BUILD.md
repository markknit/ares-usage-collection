...
3. Configure the common OneDrive remote.
4. Create one Round Sync **Copy** task:
   - Source: `Download/ARES-Usage`
   - Destination: `ARES-OneDrive:ARES Usage Uploads/Incoming`
   - Keep only ARES usage and setup-test files in the source folder so no task filter is required.
5. Run the task manually and confirm the file appears in OneDrive.
