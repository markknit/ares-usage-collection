...
Usage:
  python3 tools/ai_task_tracker.py commit \
      --model terra --type implementation --retries 0 \
...
def cmd_commit(args: argparse.Namespace) -> int:
    ...
    # Commit only already-staged files unless --all is explicitly requested.
    git_args = ["commit"]
    if args.all:
        git_args.append("-a")
...
def cmd_export(args: argparse.Namespace) -> int:
    # Record separator and unit separator avoid ambiguity from multiline messages.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    commit.add_argument("--model", choices=MODELS, required=True)
    commit.add_argument("--type", required=True, help="Task category, e.g. implementation, debugging, config, tests, docs")
